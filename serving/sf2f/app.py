...
def inference():
    ...
    try:
        if not all([request_id, result_id, voice_url, gender, age]):
            return jsonify({"status_code": 400, "error": "Missing required parameters"})
        ...
        imagetovideo_response = requests.post("http://175.106.97.56:3001/imagetovideo", data=video_make_json, headers=headers)
